...
import { customCommandsList } from './customCommands.js';
...
const OverviewRow = ({ startWorkspace, workspaces, windowName = 'search' }) => Widget.Box({
...
export const Search = () => {
    ...
    const entry = Widget.Entry({
        ...
        onAccept: (self) => { // This is when you hit Enter
            ...
            if (isDir) {
                App.closeWindow('search');
                execAsync(['bash', '-c', `nautilus "${expandTilde(text)}"`, `&`]).catch(print);
                return;
            }
            if (_appSearchResults.length > 0) {
                ...
                for (const command of customCommandsList) {
                    ...
                    const isAlias = command.aliases.some(alias => alias.startsWith(cmdName));

                    if (isAlias || command.name.startsWith(cmdName)) {
                        command.launch(command.cmd, args);
                    }
                }
                return;
            }
            ...
            if (!isAction) {
                App.closeWindow('search');
                execAsync(['bash', '-c', `firefox 'https://duckduckgo.com/?q=${text}' &`]).catch(print)
                return;
            }
                ...
                if (entry.text == '') {
                    ...
                    if (isAction) { // Eval on typing is dangerous, this is a workaround.
                        for (const command of customCommandsList) {
                            ...
                            if (isAlias || command.name.startsWith(cmdName) || cmdName == '') {
                                resultsBox.add(
                                    CustomCommandButton(command, args)
                                );
                            }
                        }
